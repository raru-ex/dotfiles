...
async function activate(
  ...
  // iTerm2のhotkey windowを開く（閉じてる場合はAlt+Tでトグル）
  const { code: osCode, stdout: osOut, stderr: osErr } = await runCommand("/usr/bin/osascript", [
    "-e", `tell application "iTerm2"
      set isVisible to false
      repeat with w in windows
        if visible of w then set isVisible to true
      end repeat
      if isVisible then
        activate
      else
  ...
  await log(`ITERM2_ACTIVATE: code=${osCode} out=${osOut} err=${osErr}`);
