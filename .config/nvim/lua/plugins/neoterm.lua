...
-- :Tnew 実行
vim.keymap.set('n', '<C-t><C-t>', ':Ttoggle<CR>', { noremap = true, silent = true })
-- :Tnew 実行(縦分割) power lineが壊れるので使わない
-- nnoremap <silent> <C-t><C-s> :vertical Tnew<CR>
-- 非ビジュアルモード
vim.keymap.set('n', '<C-t><C-p>', ':TREPLSendLine<CR>', { noremap = true, silent = true })
-- ビジュアルモード
vim.keymap.set('n', '<C-t><C-v>', ':TREPLSendSelection<CR>', { noremap = true, silent = true })
-- ファイル送信
vim.keymap.set('n', '<C-t><C-f>', ':TREPLSendFile<CR>', { noremap = true, silent = true })
